    ...
    for line in f:
        ...
        if len(splitByComma)>1:
            ...
            if(splitByComma[0]=="const double tau_D "):
                ...
                print("tau_D = ", tau_D)    
            if(splitByComma[0]=="const double tau_m "):
                tau_m = float(splitByComma[1].strip().replace(';', ''))
                print("tau_m = ", tau_m)    
...
plt.title("Passage time, $\\tau_m$="+ str(tau_m)+"$\\tau_\Gamma$="+str(tau))
plt.xscale("log")
...
plt.title("Crossing barrier distribution, $\\tau_m$="+ str(tau_m)+"$\\tau_\Gamma$="+str(tau))
plt.scatter(hist_numb[:, 0], hist_numb[:, 1])
plt.xlabel("N-crossings")
